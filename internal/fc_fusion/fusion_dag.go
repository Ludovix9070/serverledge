...
type QueueNode struct {
	NodeID    fc.DagNodeId // ID del nodo nel DAG
	IsFusible bool         // Se il nodo è stato valutato come fusibile
	Evaluated bool         //Se il nodo è stato già valutato nei casi choice e fanout
}
...
// se sono simple node e sono next di un choice node, per essere fusibile devo essere referenziato una sola volta
// dal choice node e stop altrimenti non posso essere fuso
// controllo se sono referenziato da altri choice node o da altri nodi in generale nel dag
func CanFuseChoiceNode(node *fc.SimpleNode, dag *fc.Dag, choiceNode *fc.ChoiceNode) bool {
	// Verifica che il nodo non sia referenziato in altri rami dello stesso ChoiceNode
	/*//fa un controllo sulla funzione di un nodo non sul fatto che un nodo venga riferito oppure no
	for _, alt := range choiceNode.Alternatives {
		if alt == node.Id {
			continue
		}

		otherNode, ok := dag.Nodes[alt]
		if !ok {
			continue
		}

		if simpleNode, ok := otherNode.(*fc.SimpleNode); ok {
			if simpleNode.Func == node.Func {
				return false // Il nodo è condiviso in altri rami dello stesso ChoiceNode
			}
		}
	}*/

	// Verifica che il nodo non sia referenziato da altri ChoiceNode nel DAG
	for _, otherNode := range dag.Nodes {
		if otherChoiceNode, ok := otherNode.(*fc.ChoiceNode); ok && otherChoiceNode != choiceNode {
			for _, alt := range otherChoiceNode.Alternatives {
				if alt == node.Id {
					return false // Nodo referenziato da un altro ChoiceNode
				}
			}
		}
	}
	...
	for _, otherNode := range dag.Nodes {
		...
		if hasNext, ok := otherNode.(fc.HasNext); ok {
			for _, next := range hasNext.GetNext() {
				if next == node.Id {
					return false // Nodo referenziato da un altro nodo
				}
			}
		}
	}
...
func CanFuseFanOutNode(node *fc.SimpleNode, dag *fc.Dag, fanOutNode *fc.FanOutNode) bool {
	// Verifica che il nodo non sia referenziato in altri rami dello stesso FanOutNode
	//check solo se funzione è uguale
	/*for _, branch := range fanOutNode.OutputTo {
		if branch == node.Id {
			continue
		}

		otherNode, ok := dag.Nodes[branch]
		if !ok {
			continue
		}
		...
		if simpleNode, ok := otherNode.(*fc.SimpleNode); ok {
			if simpleNode.Func == node.Func {
				return false // Il nodo è condiviso in altri rami dello stesso FanOutNode
			}
		}
	}*/

	// Verifica che il nodo non sia referenziato da altri nodi generici nel DAG
	if IsNodeReferenced(node.Id, dag) {
		return false
	}
...
func IsNodeReferenced(nodeId fc.DagNodeId, dag *fc.Dag) bool {
	for _, otherNode := range dag.Nodes {
		// Verifica se il nodo è presente nella lista 'Next' di un nodo che implementa 'HasNext'
		if hasNext, ok := otherNode.(fc.HasNext); ok {
			for _, next := range hasNext.GetNext() {
				if next == nodeId {
					return true // Nodo referenziato
				}
			}
		}

		// Verifica se il nodo è presente nella lista di Alternative di un ChoiceNode
		if choiceNode, ok := otherNode.(*fc.ChoiceNode); ok {
			for _, alt := range choiceNode.Alternatives {
				if alt == nodeId {
					return true // Nodo referenziato in un ChoiceNode
				}
			}
		}
	}
	return false // Nodo non referenziato
}

// count references to a simple node, to be fused must be only one
func CountNodeReferences(nodeId fc.DagNodeId, dag *fc.Dag) int {
	count := 0
	...
	for _, otherNode := range dag.Nodes {
		...
		if choiceNode, ok := otherNode.(*fc.ChoiceNode); ok {
			for _, alt := range choiceNode.Alternatives {
				if alt == nodeId {
					count++ // Nodo referenziato in un ChoiceNode
				}
			}
		}
	}

	fmt.Println("dag node id ", nodeId)
	fmt.Println("counter ", count)
	return count // Nodo non referenziato
}
